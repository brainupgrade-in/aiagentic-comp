...
todo1_code = textwrap.dedent("""\
    ...
    # Counter: total tokens by model and direction (in/out)
    TOKENS = Counter(
        "agent_tokens_total",
    ...
    def record_llm_call(model, tokens_in, tokens_out, duration):
        # Record token counts
        TOKENS.labels(model=model, direction="in").inc(tokens_in)
        TOKENS.labels(model=model, direction="out").inc(tokens_out)
...
ai_queries = [
    {
        ...
        "query": "increase(agent_estimated_cost_usd[24h])",
    ...
    },
    {
        ...
        "query": "sum(rate(agent_estimated_cost_usd[5m])) / sum(rate(agent_requests_total[5m]))",
        "check_terms": ["rate", "cost", "rate", "requests"],
    },
    {
        "scenario": "LLM p99 latency",
        "query": "histogram_quantile(0.99, rate(agent_llm_duration_seconds_bucket[5m]))",
