"""
Lab 07 Solution: Sandboxed Execution
...
def sandboxed_exec(command: str, timeout: int = 10, max_output: int = 5000) -> dict:
    ...
    if not is_command_safe(command):
        return {"status": "blocked", "reason": "Command blocked by safety filter"}
...
ERROR_PATTERNS = [
    ("python_exception", r"traceback", re.IGNORECASE),
    ("generic_error", r"error:", re.IGNORECASE),
    ("permission_error", r"permission denied", re.IGNORECASE),
    ("not_found", r"not found|no such file", re.IGNORECASE),
    ("crash", r"segmentation fault|core dumped", re.IGNORECASE),
]
...
def validate_output(output: str) -> dict:
    ...
    for error_type, pattern, flags in ERROR_PATTERNS:
        match = re.search(pattern, output, flags)
        if match:
            errors.append({"type": error_type, "pattern": match.group()})

    return {"is_clean": len(errors) == 0, "errors": errors}
