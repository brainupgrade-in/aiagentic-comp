...
class ToolRegistry:
    ...
    def get_schema(self, name: str) -> dict:
        ...
        return {"name": tool["name"], "description": tool["description"], "tags": tool["tags"]}

    def discover(self, query: str = None, tags: list = None) -> list:
        """Discover tools by query text and/or tags."""
        results = []
        for tool in self._tools.values():
            # Filter by query (case-insensitive substring in description)
...
# Already have reg from validation above, so use it directly
discovered_tools = reg.discover(tags=["code_quality"])
route_result = reg.route("analyze", source="result = eval(user_input)")
