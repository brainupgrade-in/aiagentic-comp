"""
Lab 07: RAG with Citations — SOLUTION
"""
...
for q in questions:
    print(f"\nQ: {q}")
    print(f"A: {citation_chain.invoke(q)}")

# Retrieved docs inspection
...
print(f"Answer: {citation_chain.invoke(query)}")

# Filtered retrieval
print("\n--- Filtered (leave only) ---")
leave_retriever = vectorstore.as_retriever(search_kwargs={"k": 3, "filter": {"category": "leave"}})
leave_chain = (
    {"context": leave_retriever | format_docs_with_sources, "question": RunnablePassthrough()}
    | citation_prompt | llm | StrOutputParser()
)
print(f"A: {leave_chain.invoke('What are all the types of leave available?')}")

# TODO 1: Expense chain
expense_retriever = vectorstore.as_retriever(search_kwargs={"k": 3, "filter": {"category": "expense"}})
expense_chain = (
    {"context": expense_retriever | format_docs_with_sources, "question": RunnablePassthrough()}
    | citation_prompt | llm | StrOutputParser()
)
print(f"\n--- Expense chain ---")
print(f"A: {expense_chain.invoke('What can I expense?')}")

# TODO 2: Better format with category
...
print(f"\n--- Better citations ---")
print(f"A: {better_chain.invoke('Tell me about WFH policy')}")
