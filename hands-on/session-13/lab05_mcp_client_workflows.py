...
def run_client_lifecycle(server: MockMCPServer) -> Dict[str, Any]:
    ...
    Steps:
        1. Initialize — call server.handle_request("initialize")
        2. Discover  — call tools/list and resources/list
        3. Operate   — call each tool once and read each resource once
        4. Shutdown   — set server.initialized = False

    Args:
        server: A MockMCPServer instance

    Returns:
        Dict with keys: server_name, init_result, tools, resources,
        tool_results (dict of tool_name→result), resource_results (dict of uri→result),
        phases_completed (int, should be 4)
    """
...
try:
    ...
    checks = [
        ...
        db_server.initialized is False,  # shutdown completed
    ]
    if all(checks):
        score += 1
...
def run_enterprise_workflow(servers: List[MockMCPServer]) -> Dict[str, Any]:
    ...
    This simulates an enterprise scenario:
        1. Initialize all servers
        2. Discover capabilities from all servers
        3. Build a unified tool registry
        4. Execute a cross-server workflow

    Args:
        servers: List of MockMCPServer instances

    Returns:
        Dict with:
            server_count: number of servers
            all_tools: flat list of all tool names from all servers
            all_resources: flat list of all resource URIs from all servers
            unified_registry: dict mapping tool_name → server_name
            workflow_log: list of dicts with step, server, action, result_summary
    """
    # TODO: Implement the enterprise workflow.
    #
    # Hint:
    ...
    #   r1 = servers[0].handle_request("resources/read", {"uri": "db://schema"})
    #   workflow_log.append({"step": 1, "server": "postgres", "action": "resources/read",
    #                        "result_summary": "Read database schema"})
    #
    #   # Query the database
    #   r2 = servers[0].handle_request("tools/call", {"name": "query_db", "arguments": {"sql": "SELECT * FROM errors"}})
    ...
    #   r3 = servers[1].handle_request("tools/call", {"name": "search_code", "arguments": {"query": "error handler"}})
    #   workflow_log.append({"step": 3, "server": "github", "action": "tools/call",
    #                        "result_summary": "Searched code for error handler"})
    #
    #   # Create a PR with the fix
    #   r4 = servers[1].handle_request("tools/call", {"name": "create_pr", "arguments": {"title": "Fix error handling"}})
...
def route_tool_call(tool_name: str, arguments: Dict,
    ...
    Args:
        ...
        registry: Dict mapping tool_name → server_name
        servers: Dict mapping server_name → MockMCPServer

    Returns:
        Dict with: tool, server, result, routed (bool)
        If tool not found: routed=False, error message in result
    """
