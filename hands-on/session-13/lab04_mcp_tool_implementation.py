#!/usr/bin/env python3
"""
Lab 04: MCP Tool Implementation — Typed Tool Functions
...
def file_search(pattern: str, directory: str) -> List[str]:
    ...
    """
    # TODO: Walk the directory tree using os.walk().
    #   For each file found, check if it matches the pattern using fnmatch.fnmatch().
    #   Collect relative paths (relative to `directory`) of matching files.
...
def code_metrics(file_path: str) -> Dict[str, int]:
    ...
    """
    # TODO: Read the file, parse with ast, and count:
    #   - line_count:    number of lines in the file
    #   - function_count: number of ast.FunctionDef nodes (top-level + nested)
    #   - class_count:   number of ast.ClassDef nodes
    #
    # Hint:
...
try:
    ...
    # main.py has: class Calculator (add, subtract), class Logger (log), def main
    # function_count = 4 (add, subtract, log, main)
    # class_count = 2 (Calculator, Logger)
    # line_count = 16
    checks = [
...
def build_tool_schema(func) -> Dict[str, Any]:
    ...
    """
    # TODO: Build a tool schema dict with:
    #   "name":        func.__name__
    #   "description": first line of func.__doc__ (stripped)
    #   "inputSchema": {
    #       "type": "object",
    #       "properties": { ... },  # param_name -> {"type": json_type} for each param
    #       "required": [...]       # list of all parameter names
    #   }
    #
    # Hint:
    ...
    #   Skip the "return" key from hints.
