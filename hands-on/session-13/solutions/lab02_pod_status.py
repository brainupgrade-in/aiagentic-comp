...
# SOLUTION: Fill in the root cause for each status
status_cause_map["CrashLoopBackOff"] = "Application error (missing env var, wrong command, unhandled exception)"
status_cause_map["Pending"] = "Insufficient cluster resources (CPU/memory) or PVC not bound"
status_cause_map["ImagePullBackOff"] = "Wrong image name/tag or missing registry credentials"
status_cause_map["OOMKilled"] = "Container exceeded memory limit (memory leak or limit too low)"
status_cause_map["Init:0/1"] = "Init container failed (dependency not ready, wrong command)"
status_cause_map["Evicted"] = "Node under resource pressure (disk or memory)"
...
# SOLUTION: Fill in the kubectl command for each status
status_fix_map["CrashLoopBackOff"] = "kubectl logs POD --previous"
status_fix_map["Pending"] = "kubectl describe pod POD (check Events section)"
status_fix_map["ImagePullBackOff"] = "kubectl describe pod POD (check image name in Events)"
status_fix_map["OOMKilled"] = "kubectl top pods (check actual memory usage)"
status_fix_map["Init:0/1"] = "kubectl logs POD -c init-container-name"
status_fix_map["Evicted"] = "kubectl describe node NODE (check Conditions)"
