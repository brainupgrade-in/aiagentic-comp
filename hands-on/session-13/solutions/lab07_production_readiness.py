...
# SOLUTION: Fill in the category for each item
checklist_items["Health probes configured"] = "Reliability"
checklist_items["HPA with minReplicas >= 2"] = "Scalability"
checklist_items["Structured JSON logging"] = "Observability"
checklist_items["Non-root container"] = "Security"
checklist_items["PodDisruptionBudget defined"] = "Reliability"
checklist_items["Prometheus /metrics endpoint"] = "Observability"
checklist_items["Secrets in K8s Secrets (not env)"] = "Security"
checklist_items["Stabilization windows on HPA"] = "Scalability"
checklist_items["Rolling update strategy"] = "Reliability"
checklist_items["Resource requests AND limits"] = "Reliability"
checklist_items["Labels for app, version, team"] = "Observability"
checklist_items["Network policies"] = "Security"
...
# SOLUTION: Identify each issue
issues["Issue 1"] = "No health probes (liveness/readiness) configured"
issues["Issue 2"] = "No resource requests/limits (can OOMKill or starve other pods)"
issues["Issue 3"] = "No PDB (voluntary disruptions can kill all pods)"
issues["Issue 4"] = "Only 1 replica (single point of failure, no HA)"
issues["Issue 5"] = "Secrets in plain text env vars (GROQ_API_KEY, DATABASE_URL visible in YAML)"
issues["Issue 6"] = "No rolling update strategy (no maxSurge/maxUnavailable)"
