...
# SOLUTION: Fill in diagnosis and fix for each broken pod
broken_pods[0]["diagnosis"] = "Redis service not running/not ready — agent depends on Redis but it's not available"
broken_pods[0]["fix"] = "Deploy Redis first, or add init container to wait for Redis, or add retry logic"
broken_pods[1]["diagnosis"] = "Memory request (8Gi) exceeds available memory on all 3 nodes"
broken_pods[1]["fix"] = "Reduce memory request to fit cluster capacity, or add larger nodes"
broken_pods[2]["diagnosis"] = "Memory limit (256Mi) too low for model that needs ~420MB+ for loading"
broken_pods[2]["fix"] = "Increase memory limit to at least 1Gi (420MB model + runtime overhead)"
broken_pods[3]["diagnosis"] = "Unauthorized to pull from private registry — missing imagePullSecrets"
broken_pods[3]["fix"] = "Create docker-registry Secret and add imagePullSecrets to pod spec"
...
# SOLUTION: Create all three YAML manifests
...
# SOLUTION: Fill in the commands
cheat_sheet[0]["command"] = "kubectl get pods -o wide"
cheat_sheet[1]["command"] = "kubectl logs POD --previous"
cheat_sheet[2]["command"] = "kubectl describe pod POD"
cheat_sheet[3]["command"] = "kubectl top pods"
cheat_sheet[4]["command"] = "kubectl exec -it POD -- bash"
cheat_sheet[5]["command"] = "kubectl get events --sort-by=.lastTimestamp"
cheat_sheet[6]["command"] = "kubectl get endpoints SERVICE"
cheat_sheet[7]["command"] = "kubectl describe node NODE"
