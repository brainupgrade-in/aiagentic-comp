#!/usr/bin/env python3
"""
Lab 07: MCP Client — Discovery and Tool Workflows
...
class MockMCPClient:
    ...
    Attributes:
        ...
        server_name (str): Name received during initialization
        tools (list): List of tool definitions from the server
    """
    ...
    def connect(self) -> bool:
        ...
        """
        # TODO: Set self.connected to True, set self.server_name to
        #   "code-assistant-server", and return True.
    ...
    def list_tools(self) -> List[Dict]:
        ...
        Returns:
            List of tool definitions (uses SERVER_TOOLS global)

        Raises:
            RuntimeError: If not connected
        """
        # TODO: If not self.connected, raise RuntimeError("Not connected").
        #   Otherwise, set self.tools = SERVER_TOOLS and return self.tools.
    ...
    def call_tool(self, name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Raises:
            RuntimeError: If not connected
            ValueError: If tool name is not in discovered tools
        """
        # TODO:
        #   1. If not connected, raise RuntimeError("Not connected")
        #   2. Check that `name` is in self.tools (by checking tool["name"])
        #      If not found, raise ValueError(f"Unknown tool: {name}")
        #   3. Call _simulate_tool_call(name, arguments) to get the result
        #   4. Append {"tool": name, "arguments": arguments, "result": result}
        #      to self._call_log
        #   5. Return the result
    ...
    def disconnect(self) -> bool:
        ...
        Returns:
            True if disconnection succeeds
        """
        # TODO: Set self.connected to False, return True.
...
def run_workflow(client: MockMCPClient) -> Dict[str, Any]:
    ...
    Steps:
        ...
        3. Search code for "import" statements
    ...
    Returns:
        Dict with keys: tools_found, search_results, test_results,
        dep_results, steps_completed
    """
...
try:
    ...
    checks = [
        ...
        wf_client.connected is False,  # should be disconnected
    ]
    if all(checks):
        score += 1
...
def tool_selector(task_description: str, available_tools: List[Dict]) -> Optional[str]:
    ...
    Strategy: For each tool, check if any word from the tool's description
    appears in the task_description (case-insensitive). Pick the tool with
    the most keyword matches.

    Args:
        task_description: Natural language description of the task
        available_tools: List of tool dicts with "name" and "description"

    Returns:
        The name of the best matching tool, or None if no match.
    """
