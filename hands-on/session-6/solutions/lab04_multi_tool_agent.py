"""
Lab 04: Multi-Tool Agent — SOLUTION
"""
...
print("=" * 50)
print("  Multi-Tool Agent — SOLUTION")
print("=" * 50)
...
# TODO 1: WFH policy tool
@tool
def get_wfh_policy() -> str:
    """Get UniGPS Work From Home policy including eligibility, schedule, and requirements."""
    return ("WFH Policy: Eligible after 6-month probation. Up to 3 days/week with team lead approval. "
            "Friday is mandatory in-office. Core hours: 10 AM - 4 PM IST. "
            "Must be on Slack and email during core hours. VPN required for internal systems. "
            "New employees must work from office for the first 6 months.")

agent = create_react_agent(
    llm, [check_leave_balance, get_office_info, calculate_expense, get_tech_stack, get_wfh_policy]
)
print("Multi-tool agent ready (5 tools)!\n")
...
print("\n--- TODO 2: Triple-Tool Question ---")
triple_q = ("I'm Anita Desai. I need my leave balance, the Mumbai office "
            "address, and what frontend framework to use for a new project.")
print(f"Q: {triple_q}")

response = agent.invoke({"messages": [("user", triple_q)]})
tool_count = sum(1 for m in response["messages"] if m.type == "tool")
