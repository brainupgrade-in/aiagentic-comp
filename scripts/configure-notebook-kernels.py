#!/usr/bin/env python3
"""
Configure Jupyter notebook kernels for all solution notebooks.
...
def main():
    ...
    solution_notebooks = list(hands_on_dir.glob('*/solutions/*.ipynb'))

    if not solution_notebooks:
        print("No solution notebooks found!")
        sys.exit(1)

    print(f"Found {len(solution_notebooks)} solution notebooks")
    print("Updating kernel configuration...\n")

    success_count = 0
    failed_count = 0

    for notebook_path in sorted(solution_notebooks):
    ...
    print(f"Total: {len(solution_notebooks)}")
